...
\item Las bases de datos relacionales tiene un atributo obligatorio (columna ó combinación
...
predictivo que el loess, sin embargo la interpretabilidad de este último es demasiado compleja (por no decir que no se puede interpretar). 
...
```{r  fig.align='center', fig.cap='Diagrama de cajas de afh vs género'}
datos <- read.csv("datosage_6.csv", header = T) %>%
  mutate(gender = as.factor(gender))
...
```{r fig.align='center', fig.cap='Histograma de afh'}
ggplot(data = datos, aes(x = afh)) +
  geom_histogram(binwidth = 5, 
                 color = "darkgreen", fill = "lightgreen") +
  ylab("Frecuencia") +
